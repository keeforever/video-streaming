...
const roomInputLocal = document.getElementById("room-input-local")
const roomInputRemote = document.getElementById("room-input-remote")

const roomGenerateBtn = document.getElementById("room-generate-btn")
const invitationBtn = document.getElementById("room-invite-btn")
const roomAcceptBtn = document.getElementById("room-accept-btn")
...
const remoteSignal = async (offer)=>{
  ...
  remotePeerConnection.onicecandidate = (event)=>{
    if(event.candidate){
      ...
      socket.emit( "room", {roomID : roomInputRemote.value, answer})
    }
  }
  ...
  remotePeerConnection.setRemoteDescription(JSON.parse(offer))
...
const getInviteBtn = ()=>{
  if(roomInputLocal.value){
    socket.emit("room", {
      roomID : roomInputLocal.value,
      offer
    })
...
// room
roomGenerateBtn.onclick = generateRoomId
roomAcceptBtn.onclick = acceptRoomId
invitationBtn.onclick = getInviteBtn
